...
import 'package:searcher_installer_go/helpers/charts/chart_source/bezier/my_single_child_scroll_view.dart';
...
class BezierChartState extends State<BezierChart>
  ...
  ///Refresh the position of the vertical/bubble from pointer event
  void refreshPointerPosition(PointerHoverEvent event, pointerEnterStatus) {
    if (_animationController.status == AnimationStatus.completed &&
        _displayIndicator &&
        pointerEnterStatus) {
      return _updatePosition(event.position);
    }
  }

  position(event) {
    _updatePositionData(event).then((data) => setState(() {
          _verticalIndicatorPosition = data;
        }));
  }

  Future<dynamic> _updatePositionData(event) async {
    return await _updatePosition(event.position);
  }

  ///Update and refresh the position based on the current screen
  _updatePosition(Offset globalPosition) {
    RenderBox renderBox = context.findRenderObject();
    final position = renderBox.globalToLocal(globalPosition);
    if (position == null) return null;
    var fixedPosition = Offset(
  ...
  _handlePointerEnter(PointerEnterEvent event) {
//    _pointerIndex++;
    print("Enter pointer status received");

    if (!_displayIndicator) {
//      print("Pointer Index: $_pointerIndex");
      // onPointerHover = true;
      _displayIndicator = true;
      _animationController.forward(
        from: 0.0,
...
//    if (onPointerHover) {
    print("Hover pointer status received");
    _onDataPointSnap(double.maxFinite);
    _updatePositionData(event.position);
    //}
  }

  _handlePointerExit(PointerExitEvent event) {
//    if (_pointerIndex == 0)
//      return;
//    else {
    print("Exit pointer status received");
//      print("Pointer Index: $_pointerIndex");
    _onHideIndicator();
    // }
  }

  _handlePointerExitRepeat(PointerExitEvent event) {
    print("Exit pointer repeat status received");
    print("Pointer Index: $_pointerIndex");

    if (_pointerIndex == 0 && onPointerHover)
      onPointerHover = false;
    else if (_pointerIndex == 0 && !onPointerHover) {
      print("Exit pointer status received");
      print("Pointer Index: $_pointerIndex");
      _onHideIndicator();
    }
  ...
  onHoverDisplayIndicator(event, pointerEnterStatus) {
    if (!_displayIndicator) {
      if (pointerEnterStatus && !onPointerHover) {
        _animationController.forward(
          from: 0.0,
        );
        _displayIndicator = true;
...
//  }
  ...
  Widget _pointerListener(Widget child) {
    return Container(
        child: _listener = Listener(
      onPointerEnter: widget.config.mouseHover // && _pointerIndex >= 0
          ? (event) => _handlePointerEnter(event)
          : null,
      onPointerHover: widget.config.mouseHover // && _pointerIndex > 0
          ? (event) => position(event)
          : null,
      onPointerExit: widget.config.mouseHover // && _pointerIndex != 0
          ? (event) => _onHideIndicator()
          : (event) => null,
      onPointerDown: (_) {
        _touchFingers++;
        if (_touchFingers > 1) {
      ...
      builder: (context, constraints) {
        ...
        if (widget.config.displayYAxis) {
            ...
            items.add(Positioned(
              ...
              child: Container(
                ...
                decoration: widget.config.backgroundGradient != null
                    ? BoxDecoration(gradient: widget.config.backgroundGradient)
                    : null,
                color: widget.config.backgroundGradient != null
                    ? null
                    : widget.config.backgroundColor,
          ...
          final maxValue = _yValues.last -
              (widget.config.startYAxisFromNonZeroValue ? _yValues.first : 0.0);
          final steps =
              widget.config.stepsYAxis != null && widget.config.stepsYAxis > 0
